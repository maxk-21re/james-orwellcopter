...
class ClusterService @Inject() (
  ...
  val publishActor: ActorRef = connection.createChannel(ChannelActor.props(setupPublisher), Some("publisher"))
  ...
  val subscribeActor: ActorRef = connection.createChannel(ChannelActor.props(setupSubscriber), Some("subscriber"))
  ...
  def calculate(logs: Seq[Geolog]): Unit = {
      ...
      def publish(channel: Channel) {
        channel.basicPublish(exchangeName, "CALC_CLUSTER_START", null, payload.toString().getBytes("UTF-8"))
      }
