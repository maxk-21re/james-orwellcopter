...
@Singleton()
class ClusterDAO @Inject() (protected val dbConfigProvider: DatabaseConfigProvider)(implicit executionContext: ExecutionContext) extends HasDatabaseConfigProvider[JdbcProfile] {
  ...
  def toCluster(raw: RawCluster): Cluster = new Cluster(raw.mbr.getEnvelopeInternal(), raw.location, raw.adress, raw.id)
  def toCluster(raw: Seq[RawCluster]): Seq[Cluster] = raw.map( e => new Cluster(e.mbr.getEnvelopeInternal, e.adress, e.adress, e.id))
  
  def toRawCluster(cluster: Cluster): RawCluster = new RawCluster(new GeometryFactory().toGeometry(cluster.mbr), cluster.location, cluster.adress, cluster.id)
  def toRawCluster(clusters: Seq[Cluster]): Seq[RawCluster] = 
    clusters.map(cluster => new RawCluster(new GeometryFactory().toGeometry(cluster.mbr), cluster.location, cluster.adress, cluster.id))
