...
object Geolog {
  implicit object timestampFormat extends Format[LocalDateTime] {
    ...
    def reads(json: JsValue) = {
      ...
      JsSuccess(LocalDateTime.ofInstant(Instant.ofEpochMilli(format.parse(str).getTime),ZoneId.of("Europe/Berlin")))
    }
    
    def writes(ts: LocalDateTime) = JsString(ts.format(DateTimeFormatter.ofPattern("yyyy-MM-dd'T'HH:mm:ss.SS'Z'")))
  }
